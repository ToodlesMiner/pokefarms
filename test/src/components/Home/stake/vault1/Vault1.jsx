...
const Vault1 = ({ lp0Token, pool, contract, user, connectWallet }) => {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        const blastStaked = blastValuePerlp1Tokens * pool0Balance;
  ...
  return (
    <div className={stl.vault}>
      ...
      <div className={stl.titleBox}>
        ...
        <h2>{lp0Token?.baseToken?.symbol}/PLS LP</h2>
        <span>
          <FaRegCopy
            className={stl.copy}
            onClick={() =>
              handleCopyAddress(`${lp0Token.baseToken.symbol} LP`, pool.LP0)
            }
          />{" "}
          {pool.LP0}
        </span>
