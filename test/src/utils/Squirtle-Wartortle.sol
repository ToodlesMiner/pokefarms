...
//WARNING!!!!!! testing purpuses only contract has admin keys not intended for public use *********************
...
// import "./tWartortle.sol";
// //import "./fast_slow.sol";
// import "@openzeppelin/contracts/utils/math/SafeMath.sol";
// import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract tTrainer is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of tokenA
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime;  // Last time that tokenA distribution occurs.
        uint256 accRewardPerShare; // Accumulated tokenA per share, times 1e12. See below.
    }

    tTokenB public Token_B;

    // Reward tokens created per second.
    uint256 public RewardPerSecond;

    // set a max reward per second, which can never be higher than 1 per second
    uint256 public constant maxRewardPerSecond = 1000e18; //@dev adjust for differant token supply. 
    ...
    // The block time when tokenB mining starts.
    ...
    event SetRewardPerSecond(address indexed user, uint256 newRewardPerSecond);
    event AddPool(address indexed user, uint256 indexed pid, address lpToken, uint256 allocPoint);
    event SetPool(address indexed user, uint256 indexed pid, uint256 newAllocPoint);
    event MakerChanged(address indexed oldMaker, address indexed newMaker1, address indexed newMaker2);
    ...
    function calculateRatio() public view returns (uint256) {
        ...
        uint256 computedRatio = _initialRatio; // Start with initial ratio
        
        if (isInflating) {
            computedRatio = _initialRatio / 10;

            // Apply compounded inflation for Phase 2
            for (uint256 i = 0; i < _InflationCount; i++) {
                computedRatio = computedRatio + (computedRatio * _InflationPercent) / 1e4;
                
                if (computedRatio >= _initialRatio) { 
                    computedRatio = _initialRatio;
                    break; 
                }
            }
        } else {
            ...
            for (uint256 i = 0; i < _InflationCount; i++) {
                computedRatio = (computedRatio * (1e4 - _InflationPercent)) / 1e4;

                
                if (computedRatio >= _initialRatio / 10) { 
    ...
    // Changes TokenA token reward per second, with a cap of maxRewardPerSecond per second
    // Good practice to update pools without messing up the contract
    function setRewardPerSecond(uint256 _RewardPerSecond) external onlyOwner {
        require(_RewardPerSecond <= maxRewardPerSecond, "setSecPerSecond: too many SEC!");

        // This MUST be done or pool rewards will be calculated with new Reward per second
    ...
    function changerMaker (address newMaker1, address newMaker2) external  onlyOwner {
        Token_B.changeMaker(newMaker1, newMaker2);
    ...
    // Safe INC transfer function, just in case if rounding error causes pool to not have enough tokenA.
    ...
    function SetInflationCount(uint256 _amount) external onlyOwner {
        Token_B.SetInflationCount(_amount);
    }

    // function mint1(uint256 _amount) external {
    //     require(_amount > 0, "Amount must be greater than zero");        
        
    //     IERC20(Token_A).approve(address(Token_B), _amount);
        
    //     address caller = msg.sender;    
    //     Token_B.Mint1(caller, _amount);
