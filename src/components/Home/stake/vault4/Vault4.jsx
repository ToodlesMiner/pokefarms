import stl from "./Vault4.module.css";
...
const Vault4 = ({
    ...
    const initialize = async () => {
      try {
        if (user) {
          ...
          const currentReward = Number(await contract.pendingReward(3, user));
        ...
        const allocPoints = await contract.poolInfo(3);
  ...
  const stake = async () => {
    ...
    try {
      ...
      const hasApproved = localStorage.getItem(
        `Vault4Approved:${pool.trainerContract}:${user}`
      );
      if (!hasApproved) {
      ...
      const depositTx = await contractWithSigner.deposit(3, inputAmountWei);
  ...
  const unStake = async () => {
    ...
    try {
      ...
      const withdrawTx = await contractWithSigner.withdraw(3, amount);
  ...
  const claimReward = async () => {
    ...
    try {
      ...
      const withdrawTx = await contractWithSigner.withdraw(3, 0);
      await withdrawTx.wait();
